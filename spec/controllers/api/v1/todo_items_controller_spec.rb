  ...
  describe 'create' do
    ...
    context 'when authenticated' do
      ...
      it 'returns a message if invalid' do
        ...
        expect { post :create, format: :json, params: { todo_item: invalid_new_todo } }.not_to change(TodoItem, :count)
        expect(response.status).to eq(422)
      end

      it "does not allow a user to create other's todo_items" do
        sign_in user_with_todo_items
        new_todo = { title: 'a new todo create by the wrong account', user: another_user_with_todo_items }
        post :create, format: :json, params: { todo_item: new_todo }
    ...
    context 'when not authenticated' do
      it 'returns unauthorized' do
        new_todo = { title: 'a new todo item', user: user_with_todo_items }
        post :create, format: :json, params: { todo_item: new_todo }
        expect(response.status).to eq(401)
      end
    end
  ...
  describe 'destroy' do
    ...
    context 'when authenticated' do
      ...
      it "does not allow a user to destroy other's todo_items" do
        ...
        expect { delete :destroy, format: :json, params: { id: another_users_destroyed_todo.id } }.not_to change(TodoItem, :count)
      end
    end
